...
angular.module('ovh-angular-contracts').run(['$templateCache', function($templateCache) {
  ...
  $templateCache.put('ovh-angular-contracts.html',
    ...
    "                    </pre></uib-accordion-group></uib-accordion></div><div class=\"contracts-breadcrumb my-2 text-center clearfix\" role=navigation data-ng-show=\"ContractsCtrl.contracts.length > 1\"><a class=\"pull-left contracts-breadcrumb-navigate contracts-breadcrumb-navigate-previous\"><i class=\"glyphicon glyphicon-chevron-left\" aria-hidden=true></i> <span data-translate=contract_previous></span></a><ul class=\"contracts-menu list-unstyled\"><li class=contracts-breadcrumb-navigate data-ng-repeat=\"contract in ContractsCtrl.contracts track by $index\"><a title=\"{{ contract.name }}\" data-fake-href=\"#contract-{{ $index }}\" data-ng-class=\"{\n" +
    "                            'active' : ContractsCtrl.currentContract.name == contract.name\n" +
    "                       }\"></a></li></ul><a class=\"pull-right contracts-breadcrumb-navigate contracts-breadcrumb-navigate-next\"><span data-translate=contract_next></span> <i class=\"glyphicon glyphicon-chevron-right\" aria-hidden=true></i></a></div><a class=\"d-block text-center my-2\" title=\"{{ contract.name }}\" target=_blank rel=noopener data-ng-if=ContractsCtrl.currentContract.name data-ng-href=\"{{ ContractsCtrl.currentContract.url || ContractsCtrl.currentContract.pdf }}\"><i class=\"fa fa-file-pdf-o\" aria-hidden=true></i> <span data-translate=contract_download data-translate-values=\"{ 'id': ContractsCtrl.currentContract.name }\"></span></a><div class=checkbox><label><input type=checkbox data-ng-model=\"ContractsCtrl.agree\"> <span data-ng-if=\"ContractsCtrl.contracts.length > 1\" data-translate=contracts_agree></span> <span data-ng-if=\"ContractsCtrl.contracts.length === 1\" data-translate=contract_agree></span></label></div></div></div>"
  );
